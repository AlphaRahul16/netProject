...
public class GCS_OMR_Test extends BaseTest {
	...
	public void Step06_Verify_Member_Can_Renew_For_Multiple_Years_After_Selecting_Currency_As_INR() {
		...
		mapRenewedProductDetails = test.asm_OMR.addMembershipsForRegularMember(mapGcsOMR);
	...
	public void Step08_Enter_Details_On_Bank_Payment_Page_And_Process_Further_Simulation() {
		String name = (memDetails.get(0).split(" ")[1] + " " + memDetails.get(0).split(" ")[0]);
		System.out.println(name);
		test.gcsPaymentPage.EnterDetailsOnBankPaymentPageAndProcessFutherSimulation(mapGcsOMR.get("Card_Type"),
	...
	public void Step11_Navigate_To_Payments_And_Verify_Global_Constituent_System_Log() {
		...
		test.invoicePage.verifyGlobalConstituentSystemLogForGCSOMR("ACS_OMR",
				mapGcsOMR.get("Card_Type").replace("Card", "").toUpperCase().trim());
