...
public class ACS_Store_Test {
	...
	public void Step01_TC01_Search_Product_Using_Product_key() {
		...
		test.homePageIWEB.enterAuthentication(getACSStore.getAuthenticationInfo("userName"),
	...
	public void Step02_TC02_Navigate_To_Shopping_Cart_Page_And_Verify_Product_Details() {
		String caseId = getStoreCaseId.get_ACSStoreCaseId("CASEID");
		String Product_id = test.homePageIWEB.getACS_Store_SheetValue(caseId, "Product_id");
		test.launchApplication(app_url_Store);
		test.asm_storePage.searchText(Product_id);
		test.asm_storePage.verifySearchSuccessfully();
		test.asm_storePage.NavigateToShoppingCartPage(memberStoreDetails.get(1),
				getACSStore.getACSStoreInfo("password"));
		test.asm_storePage.verifyUserIsOnShoppingCartPage();
		mapGetMemberDetailsInStore = test.asm_storePage.verifyProductDetailsOnShoppingCartPage(caseId, "priceValue",
	...
	public void Step03_TC03_Navigate_To_Secure_Checkout_Page_And_Get_Prepopulated_Address_Feilds() {
		String caseId = getStoreCaseId.get_ACSStoreCaseId("CASEID");
		String Address = test.homePageIWEB.getACS_Store_SheetValue(caseId, "Address");
		String City = test.homePageIWEB.getACS_Store_SheetValue(caseId, "City");
		String ZipCode = test.homePageIWEB.getACS_Store_SheetValue(caseId, "ZipCode");
		String State = test.homePageIWEB.getACS_Store_SheetValue(caseId, "State");
		...
		test.asm_storePage.selectAndEnterNewShippingAddress(mapShippingAddressDetails.get("First Name"),
				mapShippingAddressDetails.get("Last Name"), Address, City, State, ZipCode);

	}

	@Test
	public void Step04_TC04_Fill_Payment_Details_On_Payment_Information_Page() {
		test.asm_storePage.enterPaymentInformation_ACSStore(getACSStore.getCreditCardInfo("Type"),
				mapShippingAddressDetails.get("First Name") + " " + mapShippingAddressDetails.get("Last Name"),
				getACSStore.getCreditCardInfo("Number"), getACSStore.getCreditCardInfo("CreditCardExpiration"),
	...
	public void Step05_TC05_Verify_Order_Summary_And_Place_The_order() {
		String caseId = getStoreCaseId.get_ACSStoreCaseId("CASEID");
		String Shipping = test.homePageIWEB.getACS_Store_SheetValue(caseId, "Shipping?");
		...
		test.asm_storePage.verifyOrderSummaryAtCheckoutPage(caseId, Shipping,Tax,orderTotal);
		test.asm_storePage.clickPlaceYourOrder();
		test.asm_storePage.verifyThankyouMessageAfterOrderCompletion(
				getACSStore.getACSStoreInfo("maxWaitTimeForPrintMessage"), "Print Your Receipt");
	}
