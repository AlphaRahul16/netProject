...
public class ACS_My_Account_Test extends BaseTest {
	...
	String app_url_IWEB, app_url_MyAccount, app_url_MyAccount_newUser, userName, customerId, telephoneType,
			techDivision, webLogin;
	int number;
	...
	public void Step03_Verify_Account_Is_Registered_And_Edit_Email() {
		...
		test.acsMyAccount.verifyUserName(userName, test.acsMyAccount.map().get("Last_Name"),
				test.acsMyAccount.map().get("Member?"), userName);
	...
	public void Step07_Enter_New_Password_And_Verify_Changed_Password() {
		...
		test.acsMyAccount.enterNewPassword(test.acsMyAccount.map().get("Password"),
				test.acsMyAccount.map().get("New_Password"));
		...
		test.acsMyAccount.navigateToAccountLink("My Account");
		test.acsMyAccount.clickOnSideTab("Password");
		test.acsMyAccount.enterNewPassword(test.acsMyAccount.map().get("New_Password"),
				test.acsMyAccount.map().get("Password"));
	}
	...
	public void Step09_Verify_Telephone_Number_And_Address_Details() {
		test.memberShipPage.expandDetailsMenuIfAlreadyExpanded("telephone numbers");
		test.memberShipPage.verifyTelephoneDetails("telephone numbers", changedValues.get("TelephoneType"),
				changedValues.get("PhoneNumber"));
		test.memberShipPage.expandDetailsMenuIfAlreadyExpanded("addresses");
