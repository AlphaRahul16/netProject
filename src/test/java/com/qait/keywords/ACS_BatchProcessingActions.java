...
public class ACS_BatchProcessingActions extends ASCSocietyGenericPage {
	...
	public void verifyDetailsOnBatchSummaryInfo(String type, String batchTotal) {
		...
		Assert.assertTrue(
				elements("txt_batchDetailsSummary", type).get(0).getText()
						.equals(elements("txt_batchDetailsSummary", type).get(1).getText()),
				"ASSERT FAILED: Control " + type + " on batch summary info should equal to Batch " + type + "");
	...
	private void verifyClosedByAndPostByDateNotEmpty(String fieldName) {
		...
		Assert.assertTrue(element("txt_postedClosedUserDate", fieldName).getText().length() != 1,
				fieldName + " date is null");
	...
	private void verifyBatchSaleRequestAs(String field) {
		...
		Assert.assertTrue(element("txt_closeDateAndSaleRequest", field).getText().equalsIgnoreCase("Yes"),
				"Batch Sale request is not Yes");
		;
	...
	public void verifyRefundDetails(ArrayList<String> refundInfo, String type) {
		...
		for (WebElement ele : elements("tbl_RefundTotal", type)) {
			...
			Assert.assertTrue(refundInfo.contains(ele.getText().trim()),
					"amount " + ele.getText().trim() + "not present on " + type + " page\n");
