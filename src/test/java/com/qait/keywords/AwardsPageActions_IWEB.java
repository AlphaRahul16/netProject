...
public class AwardsPageActions_IWEB extends ASCSocietyGenericPage {
	...
	public void editWinnerNomineesFromJudges() {
		...
		if (checkIfElementIsThere("btn_editChild", "acs award winner")) {
			...
			if (sizeOfNominees > 0) {
				...
				int sizeOfEditButton = elements("btn_editChild",
						"acs award winner").size();
				for (int i = 1; i <= sizeOfEditButton; i++) {
					// for (WebElement ele :
					// elements("btn_editChild","acs award winner")) {
					// ele.click();
					if (verifyValidField(j)) {
						elements("btn_editChild", "acs award winner").get(j)
								.click();
						wait.hardWait(1);
						wait.waitForPageToLoadCompletely();
						switchToFrame("iframe1");
	...
	public boolean verifyValidField(int j) {
		...
		try {
			...
			wait.resetExplicitTimeout(hiddenFieldTimeOut);
			isElementDisplayed("img_valid", "acs award winner",
					String.valueOf(j + 1));
			wait.resetImplicitTimeout(timeOut);
			wait.resetExplicitTimeout(timeOut);
			if (element("img_valid", "acs award winner", String.valueOf(j + 1))
					.isDisplayed()) {
