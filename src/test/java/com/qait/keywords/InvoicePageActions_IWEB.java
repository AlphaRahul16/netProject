...
public class InvoicePageActions_IWEB extends ASCSocietyGenericPage {
	...
	public void verifyInvoiceDetailsGCSOMA(String invoiceNumber, String Total,
			String paidInFull) {
	...
	public void verifyInvoicedDetails(String caseId, String menuName,
		...
		expandDetailsMenu("line items");
		hardWaitForIEBrowser(10);
		verifyInvoiceDetails("priceValue", map().get("Iweb Product Name?"),
				getPriceSheetValue(caseId, "Price value?"),
				map().get("multiYearDecision"));
		verifyInvoiceDetails("balance", map().get("Iweb Product Name?"),
				"0.00", map().get("multiYearDecision"));
		verifyInvoiceDetails("discount", map().get("Iweb Product Name?"),
				"0.00", map().get("multiYearDecision"));

		verifyInvoiceDetails("priceValue", map().get("Iweb Pub Name?"),
				getPriceSheetValue(caseId, "pubsPrice?"), "1");
		verifyInvoiceDetails("discount", map().get("Iweb Pub Name?"), "0.00",
				"1");
		verifyInvoiceDetails("balance", map().get("Iweb Pub Name?"), "0.00",
				"1");

		verifyInvoiceDetails("priceValue", map().get("Iweb Division Name?"),
				getPriceSheetValue(caseId, "divisionPrice?"),
				map().get("multiYearDecision"));
		verifyInvoiceDetails("balance", map().get("Iweb Division Name?"),
				"0.00", map().get("multiYearDecision"));
		verifyInvoiceDetails("discount", map().get("Iweb Division Name?"),
				"0.00", map().get("multiYearDecision"));

		verifyInvoiceDetails("priceValue", map().get("Iweb CEN Product Name?"),
				getPriceSheetValue(caseId, "CENprice?"),
				map().get("multiYearDecision"));
		verifyInvoiceDetails("discount", map().get("Iweb CEN Product Name?"),
				"0.00", map().get("multiYearDecision"));
		verifyInvoiceDetails("balance", map().get("Iweb CEN Product Name?"),
				"0.00", map().get("multiYearDecision"));
		...
		verifyInvoiceDetails("priceValue", map().get("Iweb LS Name?"),
				getPriceSheetValue(caseId, "LSDues?"),
				map().get("multiYearDecision"));
		verifyInvoiceDetails("discount", map().get("Iweb LS Name?"), "0.00",
				map().get("multiYearDecision"));
		verifyInvoiceDetails("balance", map().get("Iweb LS Name?"), "0.00",
				map().get("multiYearDecision"));

		verifyInvoiceDetails("quantity", map().get("Iweb Product Name?"),
				quantity_Product, "1");
		verifyInvoiceDetails("quantity", map().get("Iweb Pub Name?"),
				quantity_PublicationName, "1");
		verifyInvoiceDetails("quantity", map().get("Iweb Division Name?"),
		...
		verifyInvoiceDetails("quantity", map().get("Iweb LS Name?"),
				quantity_IWEBLSName, "1");
	}
	...
	public String getDataFromInvoiceProfilePage(String field) {
		...
		String invoicedata = element("txt_invoiceValues", field).getText()
				.trim();
		return invoicedata;
	}
	...
	public void verifyScarfReviewerCommentsAndStatus(String reviewerName,
			String reviewerComment, String reviewerStatus) {
		...
		Assert.assertTrue(element("txt_code", reviewerName).getText().trim()
				.equals(reviewerComment), "reviewer " + reviewerName
				+ " comments as " + reviewerComment + " not present on iweb\n");
		logMessage("ASSERT PASSED : Reviewer " + reviewerName + "comments as "
				+ reviewerName + " is verified on iweb\n");
		Assert.assertTrue(element("txt_priceValue", reviewerName).getText()
				.trim().equals(reviewerStatus), " reviewer " + reviewerName
				+ " Status as " + reviewerStatus + " not present on iweb\n");
		logMessage("ASSERT PASSED : Reviewer " + reviewerName
				+ " Status as <b>" + reviewerStatus
	...
	public void getTableHeadingsAndVerifyPaymentValues(String tabName,
	...
	public void getParticularColumnValue(String columnName,
			List<String> tableHeadings, String tabName, String currencyValue,
		...
		verifyPaymentValues(tabName, rowIndex, currencyIndex + 3,
				currencyValue, "Currency");
		verifyPaymentValues(tabName, rowIndex, paymentModeIndex + 3,
				paymentModeValue, "Payment Mode");
	}

	public void verifyPaymentValues(String tabName, int rowIndex,
			int columnIndex, String expectedValue, String columnName) {
		Assert.assertTrue(
