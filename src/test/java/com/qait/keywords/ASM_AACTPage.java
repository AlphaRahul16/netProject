...
public class ASM_AACTPage extends ASCSocietyGenericPage {
	...
	boolean flag;
	Map<String, Object> mapASM_AACTSmoke = YamlReader
			.getYamlValues("AACT_SmokeChecklistData");
	YamlInformationProvider getKeyValueAACT = new YamlInformationProvider(
			mapASM_AACTSmoke);
	...
	String instructionalSetting = getKeyValueAACT
			.get_AACTAboutYouInfo("instructionalSetting");
	...
	public void verifyInvalidEmailFormat() {
		...
		logMessage("ASSERT PASSED : Invalid email format occured on enter invalid email id in txt_invalidEmailFormate\n");
	}
	...
	public void enterValidInformationAtAboutYouPage() {
		...
		if (country.equalsIgnoreCase("UNITED STATES")) {
			...
			wait.waitForElementToDisappear(element("list_schoolWorkInfo",
					"State"));
		}
	...
	public void verifyMemberContactDetails(String caseId) {
		...
		if (countrySelected.equalsIgnoreCase("UNITED STATES")) {
			verifySelectedDropDownValue("State",
					getAACT_OmaSheetValue(caseId, "State Contact Page"));
		}
		verifyMemberContactDetail("ZipCode",
				getAACT_OmaSheetValue(caseId, "Zip code Contact Page"));

	}

	public String verifySelectedDropDownValue(String detailType,
			String detailValue) {
		if (!detailValue.equalsIgnoreCase("null")) {
			isElementDisplayed("txt_selectedDetail", detailType);
			Assert.assertTrue(element("txt_selectedDetail", detailType)
					.getText().equalsIgnoreCase(detailValue));
	...
	public void selectMemberContactDetail(String detailType, String detailValue) {
		if (!detailValue.equalsIgnoreCase("null")) {
			...
			logMessage("Step: " + detailValue + " selected in the "
					+ detailType + "\n");
		} else {
			logMessage(detailType + " value is not present in data sheet\n");
	...
	public void verifyChemistryTeacherExpFieldVisibility(String caseId) {
		verifyFieldVisibility(
				...
				getAACT_OmaSheetValue(caseId,
						"Has Chemistry Teaching Experience?"));
	}
	...
	public void isSelectChemistryTeacher(String caseId) {
		if (getAACT_OmaSheetValue(caseId, "Teacher Member").equalsIgnoreCase(
				"Y")) {
			selectMemberType("Teacher");
			if (!getAACT_OmaSheetValue(caseId, "Address Type")
					.equalsIgnoreCase("Work/School")) {
				enterMemberContactDetail(
						"Organization",
						getAACT_OmaSheetValue(caseId,
								"Organization About You Page"));
				enterMemberContactDetail(
						"DeptMail",
						getAACT_OmaSheetValue(caseId,
								"Dept/Mail Stop About You Page"));
				enterMemberContactDetail("AddressLine1",
				...
				enterMemberContactDetail(
						"ZipCode",
						getAACT_OmaSheetValue(caseId, "Zip code About You Page"));
			} else {
				verifyMemberContactDetails(caseId);
			}

			selectDemographicDetail("Gender",
					getAACT_OmaSheetValue(caseId, "Gender"));
			wait.hardWait(5);
			wait.waitForPageToLoadCompletely();
			...
			checkMemberContactDetail(getAACT_OmaSheetValue(caseId,
		...
		else if (getAACT_OmaSheetValue(caseId, "Teacher Member")
				.equalsIgnoreCase("N")) {
	...
	public void isSelectStudentMember(String caseId) {
		if (getAACT_OmaSheetValue(caseId, "Student Member").equalsIgnoreCase(
				"Y")) {
			...
			enterDemographicDetail(
					...
					getAACT_OmaSheetValue(caseId,
							"Chemistry Teaching Experience"));
			selectDemographicDetail("GradMonth",
					getAACT_OmaSheetValue(caseId, "Expected Graduation Month"));
			wait.hardWait(5);
			wait.waitForPageToLoadCompletely();
			selectDemographicDetail("GradYear",
					getAACT_OmaSheetValue(caseId, "Expected Graduation Year"));
			wait.hardWait(5);
			wait.waitForPageToLoadCompletely();
			checkMemberContactDetail(getAACT_OmaSheetValue(caseId,
					"Instructional settings"));
			checkMemberContactDetail(getAACT_OmaSheetValue(caseId, "Grades"));
			checkMemberContactDetail(getAACT_OmaSheetValue(caseId, "Subjects"));
		} else if (getAACT_OmaSheetValue(caseId, "Student Member")
				.equalsIgnoreCase("N")) {
	...
	public void isSelectAffiliateMember(String caseId) {
		if (getAACT_OmaSheetValue(caseId, "Affiliate Member").equalsIgnoreCase(
			...
			verifyExpectedGraduationMonthFieldVisibility(caseId);
			verifyExpectedGraduationYearFieldVisibility(caseId);
		} else if (getAACT_OmaSheetValue(caseId, "Affiliate Member")
				.equalsIgnoreCase("N")) {
